      ...
      <div onClick={e=>e.stopPropagation()} style={{
        ...
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:20}}>
          ...
          <button onClick={onClose} style={{
            ...
            color: dark ? "#869489" : "#aaa",fontSize:22,lineHeight:1,padding:"2px 6px",
          }}>✕</button>
        </div>
        {children}
      </div>
...
const FAQ = [
  { q:"How does speech recognition work?",
    a:"On Chrome, Edge and Safari your browser's built-in speech engine is used — it is instant and free. On Firefox a small AI model (Whisper tiny, ~39MB) downloads once to your device and runs fully locally after that." },
  { q:"Is my audio sent to any server?",
    a:"On Chrome/Edge/Safari audio is processed by Google or Apple speech servers. On Firefox the Whisper engine runs entirely on your device — nothing leaves your browser." },
  { q:"Why is there a loading screen on Firefox?",
    a:"Firefox does not have a built-in speech API, so we download the Whisper AI model (~39MB) the first time. After that it is cached and loads instantly on every future visit." },
  { q:"What happens if I refresh the page?",
    a:"Your current transcript is saved to your browser automatically. It will be restored when you reload." },
  { q:"How do I save a session?",
    a:"Click Stop after recording. Your session is saved automatically to Recent Dictations with the title, date, word count and duration." },
  { q:"How do I pin a session?",
    a:"Go to Recent Dictations. Each entry has a pin icon on the right. Click it to pin the session to your Pinned Transcripts library. Click again to unpin." },
  { q:"What do templates do?",
    a:"Templates pre-fill the session title so your recordings are organised by type — Meeting Notes, Journal, Lecture etc. Click a template and you land on the Studio with the title already set." },
  { q:"Can I use this on mobile?",
    a:"Yes. Chrome on Android works well. A banner appears on mobile suggesting desktop for the best experience." },
  { q:"Is this free?",
...
function HelpModal({ open, onClose, dark }) {
  ...
  return (
    <Modal open={open} onClose={onClose} title="Help Centre" dark={dark}>
      ...
      {FAQ.map((item, i) => (
        <div key={i} style={{borderBottom:`1px solid ${dark?"rgba(61,74,65,0.25)":"#f0f0ee"}`,paddingBottom:12,marginBottom:12}}>
...
function Toggle({ checked, onChange, dark }) {
  return (
    <button onClick={() => onChange(!checked)} style={{
      ...
      background: checked ? "#59de9b" : (dark?"#343533":"#ddd"),
      transition:"background .2s",flexShrink:0,
    }}>
      <span style={{
        position:"absolute",top:3,left: checked?22:3,width:20,height:20,
...
function SettingsRow({ label, sub, dark, children }) {
  return (
    ...
    <Modal open={open} onClose={onClose} title="Settings" dark={dark}>
      <div style={{display:"flex",flexDirection:"column"}}>
        ...
        <SettingsRow label="Dark mode" sub="Switch between dark and light interface" dark={dark}>
          <Toggle checked={dark} onChange={v=>onChange("dark",v)} dark={dark}/>
        </SettingsRow>

        <SettingsRow label="Transcript font size" sub={`Currently ${settings.fontSize}px`} dark={dark}>
        ...
        <SettingsRow label="Auto-save sessions" sub="Save each recording to Recent Dictations on Stop" dark={dark}>
          <Toggle checked={settings.autoSave} onChange={v=>onChange("autoSave",v)} dark={dark}/>
        </SettingsRow>

        <SettingsRow label="Mic sounds" sub="Play a tone when recording starts and stops" dark={dark}>
          <Toggle checked={settings.sounds} onChange={v=>onChange("sounds",v)} dark={dark}/>
        </SettingsRow>

        <SettingsRow label="Recognition language" sub="Speech recognition language" dark={dark}>
          <select value={settings.lang} onChange={e=>onChange("lang",e.target.value)} style={{
            background:dark?"#292a28":"#f5f5f3",border:`1px solid ${dark?"rgba(61,74,65,0.4)":"#e0dfd8"}`,
            borderRadius:8,padding:"6px 10px",fontSize:13,color:dark?"#e4e2df":"#2c2c2a",
        ...
        <div style={{marginTop:20,padding:16,borderRadius:10,
          ...
          <div style={{display:"flex",gap:10,flexWrap:"wrap"}}>
            {[["Clear all history", onClearHistory],["Reset everything", onClearAll]].map(([label,fn])=>(
              <button key={label} onClick={fn} style={{
                ...
                fontSize:12,fontWeight:600,color:dark?"#ffb4ab":"#c0392b",
              }}>{label}</button>
            ))}
          </div>
        </div>
...
// ─────────────────────────────────────────────
// StopInterceptor
// listens for button clicks inside SpeechInput
// without modifying that component at all
// ─────────────────────────────────────────────
    ...
    <>
      <style>{`
        ...
        *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
        ...
        .mat{font-family:'Material Symbols Outlined';font-weight:400;
          font-variation-settings:'FILL' 0,'wght' 400,'GRAD' 0,'opsz' 24;
          font-style:normal;user-select:none;display:inline-block;line-height:1}
        ...
        .va-body{display:flex;flex:1;padding-top:64px}
        .va-main{flex:1;overflow-y:auto;min-height:calc(100vh - 64px)}

        .va-top{position:fixed;top:0;left:0;right:0;height:64px;z-index:100;
          background:${dark?"rgba(10,10,9,0.82)":"rgba(248,248,246,0.88)"};
        ...
        .va-topnav{display:flex;gap:28px}
        .va-topnav a{font-size:13px;font-weight:600;color:${onFaint};text-decoration:none;
        ...
        .va-icon-btn{width:36px;height:36px;border-radius:8px;border:none;
          background:transparent;color:${onFaint};cursor:pointer;
          display:flex;align-items:center;justify-content:center;transition:background .15s,color .15s}
        .va-icon-btn:hover{background:${bgHigh};color:${onBg}}

        .va-side{width:256px;flex-shrink:0;
        ...
        .va-new-btn{width:100%;padding:11px;margin-bottom:14px;
          background:${pri};color:${priD};border:none;border-radius:10px;cursor:pointer;
          font-family:'Manrope',sans-serif;font-size:13px;font-weight:700;
          display:flex;align-items:center;justify-content:center;gap:6px;transition:transform .15s,opacity .15s}
        .va-new-btn:hover{opacity:.9}
        .va-new-btn:active{transform:scale(.97)}
        .va-nav{display:flex;flex-direction:column;gap:2px;flex:1}
        .va-nav-item{display:flex;align-items:center;gap:12px;padding:10px 14px;
        ...
        .va-content{max-width:840px;margin:0 auto;padding:44px 28px 80px}
        ...
        .va-session-bar{display:flex;align-items:center;gap:10px;margin-bottom:14px;
          padding:10px 16px;background:${bgMid};border-radius:10px;border:1px solid ${ol}}
        .va-session-input{flex:1;border:none;background:transparent;font-family:'Manrope',sans-serif;
          font-size:14px;font-weight:600;color:${onBg};outline:none}
        .va-session-input::placeholder{color:${onFaint}}
        .va-session-tag{font-size:11px;padding:2px 8px;border-radius:20px;
          background:rgba(89,222,155,0.1);color:${pri};font-weight:700;white-space:nowrap;flex-shrink:0}

        .va-studio-card{background:${bgLow};border:1px solid ${ol};border-radius:20px;
          overflow:hidden;margin-bottom:32px;position:relative}
        .va-studio-card::before{content:'';position:absolute;top:-60px;right:-60px;width:300px;height:300px;
          background:radial-gradient(circle,rgba(89,222,155,0.04) 0%,transparent 70%);pointer-events:none}

        /* SpeechInput dark/light overrides */
        ...
        .va-stt .stt-display{background:transparent!important;padding:28px!important;min-height:210px!important;max-height:440px!important;
          color:${onBg}!important;font-size:${settings.fontSize}px!important;font-weight:300!important;line-height:1.85!important;letter-spacing:-.01em!important}
        ...
        .va-feat{padding:20px;background:${bgMid};border:1px solid ${ol};border-radius:14px;
          display:flex;align-items:flex-start;gap:14px;transition:transform .2s}
        .va-feat:hover{transform:translateY(-2px)}
        .va-feat-icon{width:40px;height:40px;border-radius:9px;background:rgba(89,222,155,0.1);
          color:${pri};display:flex;align-items:center;justify-content:center;flex-shrink:0}
        ...
        .va-entry-text{font-size:13px;color:${onMuted};line-height:1.6;
          display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;overflow:hidden}
        .va-entry-actions{display:flex;gap:4px;flex-shrink:0}
        .va-entry-btn{width:30px;height:30px;border-radius:7px;border:none;background:transparent;
          color:${onFaint};cursor:pointer;display:flex;align-items:center;justify-content:center;transition:background .15s,color .15s}
        ...
        .va-tmpl-icon{width:34px;height:34px;border-radius:8px;background:rgba(89,222,155,0.1);
          color:${pri};display:flex;align-items:center;justify-content:center;flex-shrink:0}
        ...
        .va-footer{border-top:1px solid ${ol};background:${bgLow};padding:18px 28px;
        ...
        .va-toast{position:fixed;bottom:28px;left:50%;transform:translateX(-50%);
        ...
        .va-mobile-nav{display:none;position:fixed;bottom:0;left:0;right:0;height:58px;
        ...
        .va-mobile-btn{display:flex;flex-direction:column;align-items:center;gap:2px;
          border:none;background:transparent;color:${onFaint};cursor:pointer;
          font-family:'Manrope',sans-serif;font-size:10px;font-weight:700;transition:color .15s}
      ...
      <div className="va-shell">
        ...
        <header className="va-top">
          ...
          <div className="va-top-actions">
            <button className="va-icon-btn" title={dark?"Switch to light mode":"Switch to dark mode"}
            ...
            {activeNav==="studio" && (
              <div className="va-content">
                ...
                <div className="va-session-bar">
                  ...
                  <input className="va-session-input"
            ...
            {activeNav==="recent" && (
              <div className="va-content">
                ...
                <div className="va-panel-sub">
                  Sessions are saved here automatically when you click <strong>Stop</strong>.<br/>
                  Click the <span className="mat" style={{fontSize:14,verticalAlign:"middle"}}>push_pin</span> icon on any entry to save it permanently to Pinned Transcripts.
                </div>
                {history.length===0 ? (
            ...
            {activeNav==="pinned" && (
              <div className="va-content">
                ...
                <div className="va-panel-sub">
                  ...
                  To pin: go to Recent Dictations and click the <span className="mat" style={{fontSize:14,verticalAlign:"middle"}}>push_pin</span> icon on any session.
                </div>
                {pinned.length===0 ? (
                  <div className="va-empty">
                ...
                <div className="va-tmpl-grid">
                  {TEMPLATES.map(t=>(
                    <div key={t.name} className="va-tmpl"
                      onClick={()=>{setSessionTitle(t.name);setActiveNav("studio");notify(`Template: ${t.name}`);}}>
